...
- **Docs and comments describe the present, never the history.** Every doc (`probe-*.md`, specs, plans, README) and every code/Go comment states **what is true now**, not what was true before or when it changed. Banned everywhere: "old endpoint / now replaced", "Addendum (date)", "previously X, now Y", "originally hard-required", "retired", "used to". No change-log framing, no "old vs new" comparison tables, no dated supplements tacked onto a section — fold the current truth into the body as if it had always been written that way. Git history is the record of change; the file is the record of state. (Exception: the doc header `**Date:**` / `**Server:**` provenance line, which dates the *probe observation*, not a change event.)
